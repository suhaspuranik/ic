...
import { getBoothWorkers, blockPartyWorker } from "../../apis/BoothWorkerApis";
...
const BoothWorkersPage = () => {
  ...
  const [selectedWorkerPerformance, setSelectedWorkerPerformance] =
    useState(null);
  ...
  const fetchWorkers = useCallback(async () => {
    try {
      ...
      const resp = await getBoothWorkers(partyWorkerId);
      const items = Array.isArray(resp?.RESULT) ? resp.RESULT : [];
      const mapped = items.map((w) => ({
        // UI fields retained for existing components
        id: w.party_worker_number,
        party_worker_id: w.party_worker_id,
        name: w.name,
        status: w.status,
        joiningDate: w.joining_date,
        exitDate: w.exit_date,
        boothAssigned: w.booth_id,
        boothType: null,
        address: w.address,
        history: Array.isArray(w.history) ? w.history : [],
        // full backend object fields for the profile modal
        party_worker_number: w.party_worker_number,
        gender: w.gender,
        dob: w.dob,
  ...
  const blockWorker = async (worker) => {
    if (!worker?.party_worker_id) return;
    const pid = worker.party_worker_id;
    if (blockingById[pid]) return; // guard duplicate
    try {
      setBlockingById((s) => ({ ...s, [pid]: true }));
      const resp = await blockPartyWorker(pid);

      // Robust success detection like reassignment
      ...
      const rawMessage =
        (resp &&
          (resp.p_out_mssg ??
            resp.RESULT?.p_out_mssg ??
            resp.result?.p_out_mssg)) ??
        (resultObj && (resultObj.p_out_mssg ?? resultObj.P_OUT_MSSG)) ??
        "Party Worker status updated successfully";
      const message = String(rawMessage).trim();

      const inferredSuccess =
        !flag && /success|updated|block/i.test(message || "");

      if (flag === "S" || inferredSuccess) {
        showToast(
          message || "Party Worker status updated successfully",
          "success"
        );
        await fetchWorkers();
      } else {
      ...
      setBlockingById((s) => ({ ...s, [pid]: false }));
    }
  };

  // Handle opening performance modal
  const openPerformanceModal = (worker) => {
    setSelectedWorkerPerformance(worker);
    setShowPerformanceModal(true);
  };

  // Handle closing performance modal
  ...
  // Apply filters to booth workers
  ...
  const handleReassign = (worker) => {
    const workerData = encodeURIComponent(JSON.stringify(worker));
    navigate(`/reassign-booth?worker=${workerData}`);
  };

  // Complete booth reassignment
  const completeReassignment = () => {
    if (!reassignmentData.booth) {
      alert("Please select a booth");
      return;
    }

    const updatedWorkers = boothWorkers.map((worker) => {
      if (worker.id === reassignmentData.workerId) {
        const now = new Date().toISOString().split("T")[0];
        const boothDetails = reassignmentData.booth.split(" - ");
        const boothType =
          boothDetails[1] === "Central" || boothDetails[1] === "South"
            ? "Strong"
            : boothDetails[1] === "East"
            ? "Swing"
            : "Weak";
        ...
        history.push({
          date: now,
          action: "Reassigned",
          details: `Assigned to ${reassignmentData.booth}`,
        });

        return {
          ...worker,
          status: "Active",
          boothAssigned: reassignmentData.booth,
          boothType,
          exitDate: null,
          history,
        };
    ...
    setBoothWorkers(updatedWorkers);
    setReassignmentData({ workerId: null, booth: "" });
    alert(`Worker reassigned to ${reassignmentData.booth}`);
  };
          ...
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-[var(--text-primary)]">
                {boothWorkers
                  .filter(
                    (w) => w.status === "Inactive" || w.status === "Blocked"
                  )
                  .length.toLocaleString()}
              </p>
            </div>
                    ...
                    <tr className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex space-x-2">
                          <button
                            onClick={() => toggleRowExpand(worker.id)}
                          ...
                          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div>
                              ...
                              <div className="space-y-2">
                                {/* Booth Assignment - Only show if not blocked */}
                                {worker.status !== "Blocked" && (
                                  <div className="bg-white rounded-lg border border-gray-200">
                                    {reassignmentData.workerId === worker.id ? (
                                      <div className="p-3 space-y-2">
                                        <label className="text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wide">
                                          Select Booth
                                        </label>
                                        <select
                                          value={reassignmentData.booth}
                                          onChange={(e) =>
                                            setReassignmentData({
                                              ...reassignmentData,
                                              booth: e.target.value,
                                            })
                                          }
                                          className="w-full p-2 text-sm border border-gray-300 rounded-md focus:outline-none focus:border-blue-500"
                                        >
                                          <option value="">Select Booth</option>
                                          <option value="Booth 101 - Central">
                                            Booth 101 - Central
                                          </option>
                                          <option value="Booth 102 - North">
                                            Booth 102 - North
                                          </option>
                                          <option value="Booth 103 - South">
                                            Booth 103 - South
                                          </option>
                                          <option value="Booth 104 - East">
                                            Booth 104 - East
                                          </option>
                                          <option value="Booth 105 - West">
                                            Booth 105 - West
                                          </option>
                                        </select>
                                ...
                                <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                                  <div className="grid grid-cols-1 divide-y divide-gray-200">
                                    {/* Removed Activate action as requested */}

                                    {worker.status !== "Blocked" ? (
                                      <button
                                        className="p-3 text-left hover:bg-gray-50 transition-colors group"
                                        disabled={
                                          !!blockingById[worker.party_worker_id]
                                        }
                  ...
                  <div className="space-y-3">
                    <div className="bg-gray-50 rounded-lg p-3">
                      ...
                      <p className="text-[var(--text-primary)] font-semibold">
                        {selectedWorker.party_worker_number ||
                          selectedWorker.id}
                      </p>
                    </div>
                    <div className="bg-gray-50 rounded-lg p-3">
                    ...
                    <div className="bg-gray-50 rounded-lg p-3">
                      ...
                      <p className="text-[var(--text-primary)] font-semibold">
                        {selectedWorker.email_id || selectedWorker.email || "—"}
                      </p>
                    </div>
                    <div className="bg-gray-50 rounded-lg p-3">
                      <label className="text-sm font-medium text-[var(--text-secondary)]">
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="bg-gray-50 rounded-lg p-3">
                      ...
                      <p className="text-[var(--text-primary)] font-semibold">
                        {formatDate(
                          selectedWorker.exit_date || selectedWorker.exitDate
                        )}
                      </p>
                    </div>
                    <div className="bg-gray-50 rounded-lg p-3">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className="bg-blue-50 rounded-lg p-4 text-center border border-blue-200">
                  <p className="text-2xl font-bold text-blue-600">
                    {
                      selectedWorkerPerformance.performanceMetrics
                        .tasksCompleted
                    }
                  </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-green-50 rounded-lg p-4 border border-green-200">
                  ...
                  <ul className="text-sm text-green-700 space-y-1">
                    <li>• Excellent voter outreach</li>
                ...
                <div className="bg-yellow-50 rounded-lg p-4 border border-yellow-200">
                  ...
                  <ul className="text-sm text-yellow-700 space-y-1">
                    ...
                    <li>• Could focus on difficult areas</li>
                    <li>• Digital engagement training needed</li>
                  </ul>
                </div>
                <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                  ...
                  <ul className="text-sm text-blue-700 space-y-1">
                    <li>• Assign to leadership role</li>
                    <li>• Provide advanced training</li>
                    <li>• Consider for mentorship program</li>
                  </ul>
                </div>
              </div>
